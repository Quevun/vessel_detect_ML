...
class FeatureMatMaker(object):
    ...
    def getDerivMat(self,scaled,vessel_ind,non_vessel_ind):
        ...
        """
        (x1,y1,xx1,yy1,xy1,xxx1,xxy1,xyy1,yyy1) = self.getImgDeriv(first)
        (x2,y2,xx2,yy2,xy2,xxx2,xxy2,xyy2,yyy2) = self.getImgDeriv(second)
        (x3,y3,xx3,yy3,xy3,xxx3,xxy3,xyy3,yyy3) = self.getImgDeriv(third)
        """
        (x1,y1,xx1,yy1,xy1) = self.getImgDeriv(first)
        (x2,y2,xx2,yy2,xy2) = self.getImgDeriv(second)
        (x3,y3,xx3,yy3,xy3) = self.getImgDeriv(third)
        ...
        vessel_deriv_mat = np.concatenate((u1,ux1,uy1,uxx1,uyy1,uxy1,#uxxx1,uxxy1,uxyy1,uyyy1,
                                           ...
                                           u3,ux3,uy3,uxx3,uyy3,uxy3),axis=0)#uxxx3,uxxy3,uxyy3,uyyy3),axis=0)
