...
class FeatureMatMaker(object):
    ...
    def getMat(self):
        ...
        categorized = self.categorizePixels(orient)
        feature_cube = np.zeros(shape)
        for i in range(len(self.scales)):
            scaled = getScaledImg(self.img,self.scales[i])
            for j in range(len(self.angle)):
                rotated_img,rotated_is_angle = self.rotateImg(scaled,
                                                              categorized[j],
                                                              self.angle[j])
                ind = np.nonzero(rotated_is_angle)
                                                              
                first = rotated_img[:,:,0]   #first,second and third slice in axis 2
                second = rotated_img[:,:,1]
                third = rotated_img[:,:,2]
    ...
    def rotateImg(self,img,is_angle,rot_angle):  # pads and rotates all images in the same way
        ...
        padded_is_angle = np.zeros((img.shape[0]+y_pad,img.shape[1]+x_pad,))
        ...
        rotated_is_angle = cv2.warpAffine(padded_is_angle,rot_mat,
                                            (padded_img.shape[1],padded_img.shape[0]),
...
def rotateImg(img,is_angle,rot_angle):  # pads and rotates all images in the same way
        diag_len = round(np.sqrt(img.shape[0]**2 + img.shape[1]**2))
        x_pad = round((diag_len - img.shape[1] + 5)/2)*2
        y_pad = round((diag_len - img.shape[0] + 5)/2)*2
        padded_img = np.zeros((img.shape[0]+y_pad,img.shape[1]+x_pad,img.shape[2]))
