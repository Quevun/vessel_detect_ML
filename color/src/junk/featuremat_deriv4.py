...
class FeatureMatMaker(object):
    ...
    def getMat(self):
        ...
        for i in range(len(self.scales)):
            ...
            xxxx1 = cv2.Sobel(first,cv2.CV_64F,4,0,ksize=7)[:,:,np.newaxis]
            xxxy1 = cv2.Sobel(first,cv2.CV_64F,3,1,ksize=7)[:,:,np.newaxis]
            xxyy1 = cv2.Sobel(first,cv2.CV_64F,2,2,ksize=7)[:,:,np.newaxis]
            xyyy1 = cv2.Sobel(first,cv2.CV_64F,1,3,ksize=7)[:,:,np.newaxis]
            yyyy1 = cv2.Sobel(first,cv2.CV_64F,0,4,ksize=7)[:,:,np.newaxis]
            ...
            xxxx2 = cv2.Sobel(second,cv2.CV_64F,4,0,ksize=7)[:,:,np.newaxis]
            xxxy2 = cv2.Sobel(second,cv2.CV_64F,3,1,ksize=7)[:,:,np.newaxis]
            xxyy2 = cv2.Sobel(second,cv2.CV_64F,2,2,ksize=7)[:,:,np.newaxis]
            xyyy2 = cv2.Sobel(second,cv2.CV_64F,1,3,ksize=7)[:,:,np.newaxis]
            yyyy2 = cv2.Sobel(second,cv2.CV_64F,0,4,ksize=7)[:,:,np.newaxis]
            ...
            xxxx3 = cv2.Sobel(third,cv2.CV_64F,4,0,ksize=7)[:,:,np.newaxis]
            xxxy3 = cv2.Sobel(third,cv2.CV_64F,3,1,ksize=7)[:,:,np.newaxis]
            xxyy3 = cv2.Sobel(third,cv2.CV_64F,2,2,ksize=7)[:,:,np.newaxis]
            xyyy3 = cv2.Sobel(third,cv2.CV_64F,1,3,ksize=7)[:,:,np.newaxis]
            yyyy3 = cv2.Sobel(third,cv2.CV_64F,0,4,ksize=7)[:,:,np.newaxis]
            
            first = first[:,:,np.newaxis]
            second = second[:,:,np.newaxis]
            third = third[:,:,np.newaxis]
            feature_cube[:,:,nf*i+1:nf*i+nf+1] = np.concatenate((first,x1,y1,xx1,yy1,xy1,xxx1,xxy1,xyy1,yyy1,xxxx1,xxxy1,xxyy1,xyyy1,yyyy1,
                                                                 second,x2,y2,xx2,yy2,xy2,xxx2,xxy2,xyy2,yyy2,xxxx2,xxxy2,xxyy2,xyyy2,yyyy2,
                                                                 third,x3,y3,xx3,yy3,xy3,xxx3,xxy3,xyy3,yyy3,xxxx3,xxxy3,xxyy3,xyyy3,yyyy3),
                                                                 axis=2)
    ...
    def getDerivMat(self,scaled,vessel_ind,non_vessel_ind):
        ...
        (x1,y1,xx1,yy1,xy1,xxx1,xxy1,xyy1,yyy1,xxxx1,xxxy1,xxyy1,xyyy1,yyyy1) = self.getImgDeriv(first)
        (x2,y2,xx2,yy2,xy2,xxx2,xxy2,xyy2,yyy2,xxxx2,xxxy2,xxyy2,xyyy2,yyyy2) = self.getImgDeriv(second)
        (x3,y3,xx3,yy3,xy3,xxx3,xxy3,xyy3,yyy3,xxxx3,xxxy3,xxyy3,xyyy3,yyyy3) = self.getImgDeriv(third)
        ...
        uxxxx1 = xxxx1[vessel_ind][np.newaxis,:]
        uxxxy1 = xxxy1[vessel_ind][np.newaxis,:]
        uxxyy1 = xxyy1[vessel_ind][np.newaxis,:]
        uxyyy1 = xyyy1[vessel_ind][np.newaxis,:]
        uyyyy1 = yyyy1[vessel_ind][np.newaxis,:]
        ...
        uxxxx2 = xxxx2[vessel_ind][np.newaxis,:]
        uxxxy2 = xxxy2[vessel_ind][np.newaxis,:]
        uxxyy2 = xxyy2[vessel_ind][np.newaxis,:]
        uxyyy2 = xyyy2[vessel_ind][np.newaxis,:]
        uyyyy2 = yyyy2[vessel_ind][np.newaxis,:]
        ...
        uxxxx3 = xxxx3[vessel_ind][np.newaxis,:]
        uxxxy3 = xxxy3[vessel_ind][np.newaxis,:]
        uxxyy3 = xxyy3[vessel_ind][np.newaxis,:]
        uxyyy3 = xyyy3[vessel_ind][np.newaxis,:]
        uyyyy3 = yyyy3[vessel_ind][np.newaxis,:]
                                     
        vessel_deriv_mat = np.concatenate((u1,ux1,uy1,uxx1,uyy1,uxy1,uxxx1,uxxy1,uxyy1,uyyy1,uxxxx1,uxxxy1,uxxyy1,uxyyy1,uyyyy1,
                                           u2,ux2,uy2,uxx2,uyy2,uxy2,uxxx2,uxxy2,uxyy2,uyyy2,uxxxx2,uxxxy2,uxxyy2,uxyyy2,uyyyy2,
                                           u3,ux3,uy3,uxx3,uyy3,uxy3,uxxx3,uxxy3,uxyy3,uyyy3,uxxxx3,uxxxy3,uxxyy3,uxyyy3,uyyyy3,),axis=0)
        ...
        uxxxx1 = xxxx1[non_vessel_ind][np.newaxis,:]
        uxxxy1 = xxxy1[non_vessel_ind][np.newaxis,:]
        uxxyy1 = xxyy1[non_vessel_ind][np.newaxis,:]
        uxyyy1 = xyyy1[non_vessel_ind][np.newaxis,:]
        uyyyy1 = yyyy1[non_vessel_ind][np.newaxis,:]
        ...
        uxxxx2 = xxxx2[non_vessel_ind][np.newaxis,:]
        uxxxy2 = xxxy2[non_vessel_ind][np.newaxis,:]
        uxxyy2 = xxyy2[non_vessel_ind][np.newaxis,:]
        uxyyy2 = xyyy2[non_vessel_ind][np.newaxis,:]
        uyyyy2 = yyyy2[non_vessel_ind][np.newaxis,:]
        ...
        uxxxx3 = xxxx3[non_vessel_ind][np.newaxis,:]
        uxxxy3 = xxxy3[non_vessel_ind][np.newaxis,:]
        uxxyy3 = xxyy3[non_vessel_ind][np.newaxis,:]
        uxyyy3 = xyyy3[non_vessel_ind][np.newaxis,:]
        uyyyy3 = yyyy3[non_vessel_ind][np.newaxis,:]

        non_vessel_deriv_mat = np.concatenate((u1,ux1,uy1,uxx1,uyy1,uxy1,uxxx1,uxxy1,uxyy1,uyyy1,uxxxx1,uxxxy1,uxxyy1,uxyyy1,uyyyy1,
                                               u2,ux2,uy2,uxx2,uyy2,uxy2,uxxx2,uxxy2,uxyy2,uyyy2,uxxxx2,uxxxy2,uxxyy2,uxyyy2,uyyyy2,
                                               u3,ux3,uy3,uxx3,uyy3,uxy3,uxxx3,uxxy3,uxyy3,uyyy3,uxxxx3,uxxxy3,uxxyy3,uxyyy3,uyyyy3,),axis=0)
        return vessel_deriv_mat,non_vessel_deriv_mat
    
    def getImgDeriv(self,img):
        x = cv2.Sobel(img,cv2.CV_64F,1,0)
        y = cv2.Sobel(img,cv2.CV_64F,0,1)
        xx = cv2.Sobel(img,cv2.CV_64F,2,0)
        yy = cv2.Sobel(img,cv2.CV_64F,0,2)
        xy = cv2.Sobel(img,cv2.CV_64F,1,1)
        xxx = cv2.Sobel(img,cv2.CV_64F,3,0,ksize=5)
        xxy = cv2.Sobel(img,cv2.CV_64F,2,1,ksize=5)
        xyy = cv2.Sobel(img,cv2.CV_64F,1,2,ksize=5)
        yyy = cv2.Sobel(img,cv2.CV_64F,0,3,ksize=5)
        xxxx = cv2.Sobel(img,cv2.CV_64F,4,0,ksize=7)
        xxxy = cv2.Sobel(img,cv2.CV_64F,3,1,ksize=7)
        xxyy = cv2.Sobel(img,cv2.CV_64F,2,2,ksize=7)
        xyyy = cv2.Sobel(img,cv2.CV_64F,1,3,ksize=7)
        yyyy = cv2.Sobel(img,cv2.CV_64F,0,4,ksize=7)
        return (x,y,xx,yy,xy,xxx,xxy,xyy,yyy,xxxx,xxxy,xxyy,xyyy,yyyy)
